...
public class FunctionReference extends SpelNodeImpl {
    ...
    private TypedValue myLogFunctionServiceProcess() {
        try {
            Method apply = MyLogFunctionService.class.getMethod("apply", String.class, Object.class);
            if (!processed) {
                SpelNodeImpl[] old = this.children;
                String functionName = "'" + this.name + "'";
                this.children = new SpelNodeImpl[getChildCount() + 1];
                this.children[0] = new StringLiteral(functionName, 0, 0, functionName);
                if (this.children.length - 1 >= 0) System.arraycopy(old, 0, this.children, 1, this.children.length - 1);
                processed = true;
            }
            return new TypedValue(apply);
...
//            throw new SpelEvaluationException(getStartPosition(), SpelMessage.FUNCTION_NOT_DEFINED, this.name);
            value = myLogFunctionServiceProcess();
        }
